...
var app = angular.module('starter', ['ionic','firebase','LocalStorageModule']);
app.controller('mainCtrl',  function ($scope,$rootScope,$stateParams,$state,$firebaseObject,$firebaseArray,$ionicSideMenuDelegate,$rootScope,$ionicPopup,$ionicSideMenuDelegate) {
 ...
 $scope.res =$stateParams.res || null;
//$rootScope.uid = $stateParams.res.uniqueId;
$scope.loadRes = function(){
  var refx = resRef.child($scope.res.uniqueId);
 $scope.res = $firebaseObject(refx);
}
$scope.makeBooking = function(res){
  //so now i have the restaurant 
  db.child()
  ...
  $scope.languages = ['English','Romaninan'];
  $scope.booking =  {};
  $scope.reservations = [{
